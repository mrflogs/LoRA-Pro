...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def lorapro_full_adjustment(self, partition_id):
        ...
        for idx, (grad_A_orin, grad_B_orin) in enumerate(zip(self.averaged_gradients[0][::2], self.averaged_gradients[0][1::2])):
            ...
            B = self.params_in_partition[0][2 * idx + 1]
            # projection
            delta = 1e-8     
            ...
            denom = (self.exp_avg_sq[idx].sqrt() / bias_correction2_sqrt).add_(1e-8)
            
            g = (self.exp_avg[idx] / bias_correction1) / denom
            g = g.to(B.dtype)
