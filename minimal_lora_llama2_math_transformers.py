...
def main():
    ...
    if local_rank == 0:
        wandb.init(
            entity="Neko97",
            project='LLAMA-2-7B',  
            name=f"llama-2-7b_math_rank_{lora_r}_{args.lora}_lr_{args.lr}_seed_{args.seed}",
    ...
    # Step 6: Save model
    if local_rank == 0:
        model.save_pretrained(f'./logs/transformers/llama-2-7b/math/{args.lr}/{args.lora}/{args.seed}')
        tokenizer.save_pretrained(f'./logs/transformers/llama-2-7b/math/{args.lr}/{args.lora}/{args.seed}')
        print(f"Saving at path: ./logs/transformers/llama-2-7b/math/{args.lr}/{args.lora}/{args.seed}")
